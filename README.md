...
- 👉 把一本已有小说（或自己的原稿）**拆成句子 / 分镜 / 章节**的结构化知识库，而不是只留一段摘要
...
- 批末自动触发向量增量构建 + 确定性聚合（清单恢复 / 缺章报告 / 终检 / 头文档 / 索引，无 LLM 语义判定），失败章补跑后批末自动重跑聚合
...
- 失败率 >30% 熔断停止重试；聚合 / 向量 / 索引幂等——失败重跑即恢复（确定性聚合无事务状态）
...
- `novelread/specs/`：标注规范（句子 / 分镜 / 章节标注 + 语料分析 SKILL；章节表/大事件/卷纲语义已于 2026-09-04 移除）
