 ...
 * 保护：
 *   1. 有长任务在运行（task/manager.mjs 的 hasRunningTask）→ 心跳超时【不退出】——
 *      页面被浏览器休眠/杀掉（tab discarding、内存回收）时心跳停发，任务必须跑完，不能连带被杀。
 *   2. 任务刚结束（task/manager.mjs 的 taskFinishedAt 距今 < 60s）→ 也不退出——给用户切回被休眠页面的宽限窗口。
...
export function main() {
  ...
  // 启动清理（僵尸任务标记 + stale 任务记录删除）——已抽到 task/manager.mjs
  cleanupOnStart();
