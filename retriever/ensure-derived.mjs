/**
 * ensure-derived.mjs — 调用前对比 + 自动重建（词典 + 四表）
 *
 * retrieve() 入口前调用：扫描 store 事实层现状 vs 派生数据 derivedFrom 覆盖清单，
 * 有变化 → 重建词典（entity-dict）+ 四表（lexical-index），保证检索用的是最新派生。
 *
 ...
 */
import { dictDir } from "./rag-core.mjs";
...
/**
 * 确保派生数据（词典 + 四表）最新。
 * @param {object} opts { force?: boolean } 强制重建（忽略对比）
...
export async function ensureDerived(opts = {}) {
  ...
  // 扫描一次事实层现状
  const current = scanSourceState();

  // ① 词典对比
  const dictFile = `${dictDir}/entity-dict.json`;
